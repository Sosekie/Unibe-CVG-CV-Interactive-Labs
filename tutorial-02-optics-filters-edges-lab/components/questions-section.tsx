...
function voterId() {
  const key = 'cv-tutorial-02-voter-id';
...
export function QuestionsSection() {
  ...
  const [activeGroup, setActiveGroup] = useState<(typeof groups)[number]>('Camera Models');
  ...
  return (
    <section className="question-section" id="questions" aria-labelledby="questions-title">
      <div className="question-header">
        <div><p className="section-kicker">02 / THINK &amp; DISCUSS</p><h2 id="questions-title">Which point should we unpack?</h2><p>Six selected questions cover the main ideas. Try the matching lab first, then vote for what still feels unclear.</p></div>
        <div className="question-instruction"><span>1</span> vote per browser per question · totals and released answers refresh automatically</div>
      </div>

      <div className="question-groups" role="tablist" aria-label="Question group">
        {groups.map((group) => <button key={group} role="tab" aria-selected={activeGroup === group} className={activeGroup === group ? 'active' : ''} onClick={() => setActiveGroup(group)}><strong>{group}</strong><span>{questions.filter((question) => question.groupName === group).length}</span></button>)}
      </div>
      ...
      <div className="question-grid">
        {!questions.length && !error ? <p className="loading-question">Loading the Tutorial 02 question bank…</p> : null}
        {filtered.map((question, index) => (
          <article className={question.answerPublished ? 'question-card question-card-released' : 'question-card'} key={question.id}>
            <div className="question-card-topline"><span>Q{String(index + 1).padStart(2, '0')}</span><span>{question.difficulty} · {question.votes} {question.votes === 1 ? 'vote' : 'votes'}</span></div>
            <p className="question-prompt">{question.prompt}</p>
            <div className="question-card-bottom">
              <Button type="button" aria-label={`Vote for question ${index + 1}: ${question.prompt}`} variant={question.hasVoted ? 'secondary' : 'default'} className={question.hasVoted ? 'vote-button voted' : 'vote-button'} disabled={question.hasVoted || submitting === question.id} onClick={() => submitVote(question.id)}>
                {submitting === question.id ? 'Sending…' : question.hasVoted ? 'Vote recorded' : 'I want this explained'}
              </Button>
              <span className={question.answerPublished ? 'answer-status published' : 'answer-status'}>{question.answerPublished ? 'Answer released' : 'Think first'}</span>
            </div>
            {question.answerPublished && question.answer ? <InteractiveAnswer questionId={question.id} fallback={question.answer} /> : null}
          </article>
        ))}
      </div>
    </section>
