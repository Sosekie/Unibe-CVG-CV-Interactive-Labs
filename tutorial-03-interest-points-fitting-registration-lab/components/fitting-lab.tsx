...
export function FittingLab() {
  ...
  const mapX = (x: number) => 325 + x * 52;
  const mapY = (y: number) => 220 - y * 14;
  const currentLine = method === 'ols'
    ? Number.isFinite(ols.slope) ? [{ x: -8, y: ols.slope * -8 + ols.intercept }, { x: 8, y: ols.slope * 8 + ols.intercept }] : [{ x: ols.center.x, y: -20 }, { x: ols.center.x, y: 20 }]
    ...
    <section className="lab-module" aria-labelledby="fitting-title">
      ...
      <div className="lab-layout">
        <aside className="control-panel glass-panel">
          ...
          <div className="control-block"><div className="control-label"><span>Rotate data</span><output>{rotation.toFixed(0)}°</output></div><Slider aria-label="Rotate point set" value={[rotation]} min={0} max={90} step={1} onValueChange={(value) => setRotation(Array.isArray(value) ? value[0] : value)} /></div>
          <Button variant="outline" onClick={() => { setDataset('tutorial'); setRotation(0); }}>Tutorial numerical preset</Button>
          <div className="classification"><span>{method === 'ols' ? 'y = mx + b' : 'ax + by = d'}</span><strong>{method === 'ols' ? Number.isFinite(ols.slope) ? 'm=' + ols.slope.toFixed(3) : 'vertical' : 'a=' + tls.a.toFixed(3)}</strong><small>{method === 'ols' ? Number.isFinite(ols.intercept) ? 'b = ' + ols.intercept.toFixed(3) + ' · SSE = ' + ols.error.toFixed(3) : 'normal equations are singular' : 'b = ' + tls.b.toFixed(3) + ' · d = ' + tls.d.toFixed(3) + ' · SSE⊥ = ' + tls.error.toFixed(3)}</small></div>
        </aside>
        <div className="visual-panel glass-panel">
          <div className="diagram-toolbar"><span>LINE FIT</span><span>{method === 'ols' ? 'vertical residuals' : 'orthogonal residuals'}</span></div>
          <svg className="fitting-diagram" viewBox="0 0 650 390" aria-label="Point set and fitted line">
            ...
            <g className="chart-grid">{Array.from({ length: 9 }, (_, index) => <line key={'v' + index} x1={65 + index * 65} x2={65 + index * 65} y1="25" y2="355" />)}{Array.from({ length: 7 }, (_, index) => <line key={'h' + index} x1="28" x2="622" y1={55 + index * 50} y2={55 + index * 50} />)}</g>
            ...
            <g clipPath="url(#fitClip)">
              <line className="fit-line" x1={mapX(currentLine[0].x)} y1={mapY(currentLine[0].y)} x2={mapX(currentLine[1].x)} y2={mapY(currentLine[1].y)} />
              {points.map((point, index) => {
                const projection = method === 'ols'
                  ? { x: point.x, y: Number.isFinite(ols.slope) ? ols.slope * point.x + ols.intercept : point.y }
          ...
          <div className="metric-grid fit-metrics"><article><span>OLS</span><strong>{Number.isFinite(ols.slope) ? 'm=' + ols.slope.toFixed(3) + ', b=' + ols.intercept.toFixed(3) : 'vertical / undefined'}</strong></article><article><span>TLS unit normal</span><strong>[{tls.a.toFixed(3)}, {tls.b.toFixed(3)}]ᵀ</strong></article><article><span>Centroid</span><strong>({tls.center.x.toFixed(2)}, {tls.center.y.toFixed(2)})</strong></article><article><span>Data rotation</span><strong>{rotation.toFixed(0)}°</strong></article></div>
        </div>
      </div>
